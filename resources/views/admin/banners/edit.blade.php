...
@section('content')
    <div class=" mx-auto bg-white shadow rounded p-6 mt-6">
        ...
        <form action="{{ route('banner.update', $banner->id) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
            @csrf
            <div>
                <label for="name" class="block font-medium">Section Name</label>
                <input type="text" name="section_name" id="name"
                    value="{{ old('section_name', $banner->section_name) }}"
            ...
            <div>
                ...
                <input type="text" name="section_type" id="slug"
                    value="{{ old('section_type', $banner->section_type) }}"
                    class="w-full border rounded px-3 py-2 focus:outline-none focus:ring">
                @error('section_type')
                    <p class="text-red-500 text-sm">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <div class="flex">
                    ...
                    <img src="{{ asset($images) }}" alt="" width="150px" class="mt-3 mx-1">
                    @endforeach
                </div>
